...
These instructions are shared by Amp, Claude Code, OpenCode, and Pi. `CLAUDE.md`
and `.claude/CLAUDE.md` link here; keep tool-specific settings, agents, commands,
and plugins in `.claude/` rather than replacing that directory with a symlink.
...
Reference solutions and private tests are encrypted during solver runs.
