# Rama AI Learn — repository instructions

These instructions are shared by Amp, Claude Code, OpenCode, and Pi. Root
`CLAUDE.md` links here; keep Claude-specific settings, agents, commands, and
plugins in `.claude/` rather than replacing that directory with a symlink.

Provides challenges for developing the Rama skill. Challenge READMEs and
test-support namespaces should contain only what is minimally sufficient for
an agent to solve the challenge; do not move skill guidance into a challenge.
The runner protects challenge reference solutions and private tests under its
configured paths during solver runs. Other files, such as docs or atlases, are
not automatically protected. Isolated solver runs receive an allowlisted
snapshot rather than the whole repository (including these instructions).
...
Codex uses `.agents/skills/`. Run challenges with:
`bb run-challenges --batch N --agent codex --verbose`.

## Skill Configuration

Project skills live in `.agents/skills/`. The `rama` entry links to the
publishable source in `plugins/rama-skill/skills/rama/`. Claude compatibility
entries in `.claude/skills/` link to those canonical skills; `lcb-complete`
remains a Claude command alias. OpenCode and Amp read `.agents/skills/` directly.
Current Pi documentation includes project `.agents/skills/`; verify discovery
on the installed Pi version before relying on it in a run.
...
#### Claude Code transcript script invocation

Claude's `.claude/settings.json` allowlist matches **relative paths from the
repo root** with the explicit interpreter prefix. In Claude Code, using an
absolute path or omitting the prefix won't match and may prompt for permission.
...
For transcript inspection, prefer `scripts/analyze-latest-transcript.py`.
In Claude Code, direct `grep`, `jq`, `cat`, `head`, `sed`, inline Python, or
Read/Grep calls on `.jsonl` files do not match this allowlist and may prompt
for approval. The script in its canonical form is pre-approved.
