...
Claude Code-specific guidance for this repository. The shared project rules
above are imported, not copied; keep `.claude/settings.json`, agents, commands,
and plugins in `.claude/`.
...
publishable source in `plugins/rama-skill/skills/rama/`. Claude discovers
project skills under `.claude/skills/`; targeted links there expose shared
skills without replacing Claude's settings, agents, commands or plugins.
`lcb-complete` remains a Claude command alias. Amp and OpenCode discover
`.agents/skills/` directly. Current Pi documentation includes project
`.agents/skills/` after trust; verify against the installed Pi version.
